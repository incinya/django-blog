...
from topic.views import make_topics_res
from .models import *
...
def get_topics(user):
    ...
    topics = Topic.objects.filter(author__username=user.username)
    res = make_topics_res(user, topics)
    return res
            ...
            if request.GET.keys():
                ...
                result = {'code': 200, 'username': username,
                          'data': {'info': user.info, 'sign': user.sign, 'avatar': str(user.avatar),
                                   'nickname': user.nickname, 'topics': topics}}
                return JsonResponse(result)
